...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'ligarey-app';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubMatches = onSnapshot(getColPath('matches'), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
          const data = change.doc.data();
          const id = change.doc.id;
          if (data.to === currentUser.uid) {
            setNotifications(prev => (prev.find(n => n.id === id) ? prev : [{ id, ...data, isMessage: false }, ...prev]));
    ...
    const unsubChats = onSnapshot(getColPath('chats'), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
          const data = change.doc.data();
          const id = change.doc.id;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const matchesSnap = await getDocs(getColPath('matches'));
        const chatIds = [];
        ...
        if (view === 'discover') {
          const usersSnap = await getDocs(getColPath('usuarios'));
  ...
  useEffect(() => {
    ...
    const unsubChat = onSnapshot(query(getColPath('chats'), orderBy('timestamp', 'asc')), (snapshot) => {
      const msgs = [];
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      ...
      else setAuthError('Revisa tus datos o conexión.');
  ...
  const saveProfileData = async () => {
    ...
    if (!myProfile.name.trim() || !myProfile.photo || !myProfile.phrase.trim() || !myProfile.lookingFor.trim() || myProfile.interests.length === 0) {
      setPhotoError('¡Faltan datos! Foto, Nombre, Frase, Qué buscas e Intereses son OBLIGATORIOS.');
      return false;
    }
    ...
    try {
      ...
      return true;
    } catch (e) { setPhotoError('Error al guardar. La foto es muy pesada.'); return false; }
  ...
  const deleteConversation = async (otherUserId) => {
    const snap = await getDocs(getColPath('matches'));
    snap.forEach(async (document) => {
      const d = document.data();
      if ((d.from === currentUser.uid && d.to === otherUserId) || (d.to === currentUser.uid && d.from === otherUserId)) {
        await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'matches', document.id));
      }
    });
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      handleLogout();
    } catch (e) { setAuthError('Cierra sesión y entra de nuevo para borrar la cuenta.'); }
  };
  ...
  const handleMatchAction = async (type) => {
    ...
    if (type !== 'dislike' && currentUser && !target.id.startsWith('m')) {
      await addDoc(getColPath('matches'), { 
    ...
    <div className="min-h-screen bg-stone-900 sm:bg-stone-200 flex justify-center items-center font-sans overflow-hidden">
      ...
      {newNotificationToast && (
        <div onClick={() => { setActiveChatUser({ id: newNotificationToast.from, name: newNotificationToast.fromName }); setView('chat'); setNewNotificationToast(null); }} className="fixed top-6 left-1/2 -translate-x-1/2 z-[400] w-full max-w-[340px] px-4 animate-in slide-in-from-top cursor-pointer">
          <div className="bg-stone-900 text-white p-4 rounded-3xl shadow-2xl flex items-center gap-4 border border-white/10 backdrop-blur-xl">
            ...
            <div className="flex-1"><p className="text-[9px] font-black uppercase text-rose-400">Aviso VIP</p><p className="text-xs font-bold"><b>{newNotificationToast.fromName}</b> {newNotificationToast.isMessage ? 'escribió' : 'saludó'}</p></div>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-[500] bg-stone-900/95 backdrop-blur-xl animate-in slide-in-from-bottom flex flex-col p-6">
          ...
          <div className="flex-1 relative rounded-[2.5rem] overflow-hidden shadow-2xl bg-white">
            ...
            <div className="absolute bottom-0 p-8 text-white w-full">
              <h2 className="text-4xl font-black tracking-tighter mb-1">{showInspector.name}</h2>
              <p className="text-rose-300 font-bold mb-3 italic">"{showInspector.phrase || '¡Hola!'}"</p>
              <div className="bg-white/10 rounded-2xl p-4 mb-4"><p className="text-[10px] font-black uppercase mb-1 opacity-60">Estoy buscando:</p><p className="text-sm font-medium">{showInspector.lookingFor || 'Pasarlo bien'}</p></div>
              <div className="flex flex-wrap gap-2">{(showInspector.interests || []).map(i => <span key={i} className="px-2 py-1 bg-white/20 rounded text-[10px] uppercase font-bold">{i}</span>)}</div>
            </div>
          </div>
          <button onClick={() => setShowInspector(null)} className="w-full py-5 mt-6 rounded-full bg-rose-500 text-white font-black uppercase tracking-widest active:scale-95">Cerrar Ficha</button>
        </div>
      )}
      ...
      <div className="w-full max-w-md bg-white h-screen sm:h-[850px] sm:rounded-[3rem] sm:border-[8px] sm:border-stone-800 flex flex-col relative overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 overflow-hidden relative">
            ...
            <div className="h-full flex flex-col p-6 overflow-y-auto pb-24 bg-stone-50">
              ...
              {profileMode === 'edit' ? (
                <div className="space-y-6">
                  ...
                  <input type="text" placeholder="Nombre (Obligatorio)" value={myProfile.name} onChange={e => setMyProfile({...myProfile, name: e.target.value})} className="w-full p-4 rounded-2xl border border-stone-200 outline-none" />
                ...
                <div className="flex-1 min-h-[400px] relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-stone-200 bg-stone-200">
                  ...
                  <div className="absolute bottom-0 p-6 text-white w-full"><h2 className="text-3xl font-black mb-1 leading-none">{myProfile.name || 'Tu Nombre'}</h2><p className="text-rose-300 font-bold mb-1 italic">"{myProfile.phrase || 'Tu frase'}"</p><p className="text-white/60 text-[10px] uppercase font-black tracking-widest mb-3">Busco: {myProfile.lookingFor}</p><div className="flex flex-wrap gap-2">{(myProfile.interests || []).map(i => <span key={i} className="px-2 py-1 bg-white/20 rounded text-[9px] uppercase font-bold">{i}</span>)}</div></div>
                </div>
              )}
              ...
              <div className={`flex-1 relative rounded-[2.5rem] overflow-hidden shadow-2xl bg-white border-4 ${profiles[currentIndex]?.isAlreadyMatched ? 'border-emerald-500 shadow-emerald-500/20' : 'border-stone-200'}`}>
                {profiles[currentIndex] ? (
                  <>
                    ...
                    <div className="absolute bottom-0 p-8 text-white w-full"><div className="flex items-center gap-2 mb-1"><h2 className="text-4xl font-black tracking-tighter leading-none">{profiles[currentIndex]?.name}</h2>{profiles[currentIndex]?.isAlreadyMatched && <MessageCircle className="w-5 h-5 text-emerald-400" />}</div><p className="text-rose-300 font-bold mb-1 italic">"{profiles[currentIndex]?.phrase}"</p><p className="text-white/60 text-[10px] font-black uppercase tracking-widest mb-4 leading-none">Busco: {profiles[currentIndex]?.lookingFor}</p><div className="flex flex-wrap gap-2">{(profiles[currentIndex]?.interests || []).map(i => <span key={i} className="px-2 py-1 bg-white/20 rounded text-[10px] uppercase font-bold">{i}</span>)}</div></div>
                  </>
                ) : (
            ...
            <div className="h-full flex flex-col bg-stone-50 animate-in slide-in-from-right">
              <div className="p-4 bg-white border-b flex items-center justify-between z-10 shadow-sm">
                <div className="flex items-center gap-3">
                  <button onClick={() => setView('messages')} className="p-1 active:scale-90"><ChevronLeft className="w-7 h-7 text-stone-400" /></button>
                  <div onClick={() => setShowInspector(activeChatUser)} className="flex items-center gap-3 cursor-pointer active:opacity-60 transition-opacity">
                    <img src={activeChatUser.photo || DEFAULT_AVATAR} className="w-10 h-10 rounded-full object-cover border-2 border-rose-100" />
              ...
              <form onSubmit={(e) => {
                ...
                addDoc(getColPath('chats'), { from: currentUser.uid, fromName: myProfile.name, to: activeChatUser.id, text: newMessageText, timestamp: new Date().toISOString() });
                setNewMessageText('');
          ...
          {view === 'notifications' && (
            <div className="h-full p-6 bg-stone-50 overflow-y-auto animate-in slide-in-from-right">
              <div className="flex items-center justify-between mb-6"><h2 className="text-3xl font-black uppercase tracking-tighter">Actividad</h2><button onClick={() => { setNotifications([]); setHasUnreadNotifs(false); }} className="p-2 text-stone-400 hover:text-rose-500 transition-colors flex items-center gap-1"><Eraser className="w-4 h-4" /><span className="text-[10px] font-black uppercase">Limpiar</span></button></div>
              <div className="space-y-3">
        ...
        {showDeleteConfirm && (
          <div className="absolute inset-0 z-[600] bg-black/90 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
            <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full text-center shadow-2xl">
              ...
              <div className="space-y-3"><button onClick={handleDeleteAccount} className="w-full py-4 rounded-full bg-red-500 text-white font-bold text-lg active:scale-95 transition-all">SÍ, BORRAR TODO</button><button onClick={() => setShowDeleteConfirm(false)} className="w-full py-4 rounded-full bg-stone-100 text-stone-800 font-bold text-lg active:scale-95 transition-all">CANCELAR</button></div>
            </div>
          </div>
        )}
        ...
        {showQRModal && (
          <div className="absolute inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in">
            <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full text-center relative shadow-2xl animate-in zoom-in">
              ...
              <p className="text-[10px] text-stone-500 uppercase font-black tracking-widest opacity-60 leading-none">Barra principal</p>
            </div>
          </div>
        )}
      </div>
